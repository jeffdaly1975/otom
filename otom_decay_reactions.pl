...
my %already_listed=();
my %otoms=();
my $combo_is_possible_isotope=0;
...
foreach my $line (<DATA>){
  ...
  if ($line=~/^\s*(\d+)\s+(\d+)\s+(\w+)\s*$/){
    ...
    $otom_protons_to_name{$protons}=sprintf("%2s",$name);  #NEW
...
open LIST_OUT,">otoms.decay_reactions_list.txt"  || die "error opening output list file";
...
foreach my $key_row (@sort_order){
  ...
  foreach my $key_col (@sort_order){
     ...
     $calculated_name            = $otom_protons_to_name{$result_protons    } || "..";
     $calculated_name_decay_down = $otom_protons_to_name{$result_protons - 1} || "..";

     unless (exists $already_listed{"$key_col + $key_row"}){

       print LIST_OUT "$key_row + $key_col => |⬆ $calculated_m_and_p_decay_up "   . "$calculated_name_decay_up ";
       print LIST_OUT                        "|= $calculated_m_and_p "            . "$calculated_name ";
       print LIST_OUT                        "|⬇ $calculated_m_and_p_decay_down " . "$calculated_name_decay_down\n";

       $unique_combinations++;

       unless ($calculated_name eq '..'){
        $combo_is_possible_isotope++;
       }
     }
